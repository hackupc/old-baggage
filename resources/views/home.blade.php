        ...
        @else
          ...
          <p>{{ date("j/Y H:i\h", strtotime($list[0]['created'])) }}
            @if(!empty($list[0]['deleted']))
              ...
              <a id="remove-button" href="/print/{{ $list[0]['row'] }}/{{ $list[0]['col'] }}/{{ $list[0]['id'] }}/{{ $list[0]['name'] }}/{{ $list[0]['surname'] }}/{{ strtotime($list[0]['created']) }}/{{ strtotime($list[0]['deleted']) }}/1">Reprint ticket</a>
            @else
              </p>
              @if(!empty($new))
